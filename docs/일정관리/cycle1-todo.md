# Cycle 1 내 TODO (Alan)

_사이클: **Cycle 1** (2026-07-13 ~ 2026-07-27, KST) · 담당: 김성욱(alan) · Linear team: PlatformTech (TECH)_
_최종 업데이트: 2026-07-14 · 총 7건_

> Cycle 1 설명: "Eve / Alan 은 Vonn과 함께 PT 관련 신규 프로젝트 예정"
> 이 문서는 이번 사이클에서 **내가 담당(assignee)한 이슈**만 추린 개인 TODO입니다. 상태 변경 시 체크박스와 표를 함께 갱신하세요.

## 요약

| 상태 | 건수 |
|---|---|
| 📋 Todo | 6 |
| 🟢 In Progress | 0 |
| ✅ Done | 1 |
| **합계** | **7** |

## GitOps - ArgoCD 도입 (6건)

- [ ] [TECH-126](https://linear.app/suppliesfitness/issue/TECH-126/argocd-진행) ARGOCD 진행 — No priority · 📋 Todo _(상위 트래킹 이슈)_
  - [X] ~~*[TECH-127](https://linear.app/suppliesfitness/issue/TECH-127/argocd-웹훅-연결) ARGOCD 웹훅 연결 — Medium · 📋 Todo _(TECH-126 하위)_*~~ [2026-07-13]
- [ ] [TECH-144](https://linear.app/suppliesfitness/issue/TECH-144/argocd-적용-계획) ARGOCD 적용 계획 — Medium · 📋 Todo
- [ ] [TECH-145](https://linear.app/suppliesfitness/issue/TECH-145/app-project-설계) App Project 설계 — Medium · 📋 Todo
- [ ] [TECH-146](https://linear.app/suppliesfitness/issue/TECH-146/argocd-배포-정책) ARGOCD 배포 정책 (auto-sync/self-heal/drift, vince 상의) — Medium · 📋 Todo
- [X] ~~[TECH-147](https://linear.app/suppliesfitness/issue/TECH-147/argocd-환경별-구조-잡기) ARGOCD 환경별 구조 잡기 (Helm vs Kustomize overlay)~~ — Medium · ✅ Done _(Helm 공유 차트 확정, [문서](../deployment/argocd/환경별구조-TECH-147.md))_ [2026-07-14]

## 기타 (1건)

- [ ] [TECH-128](https://linear.app/suppliesfitness/issue/TECH-128/테스트-코드-작성-및-활용) 테스트 코드 작성 및 활용 — Medium · 📋 Todo · label: Improvement
