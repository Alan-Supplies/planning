...
> 소스 오브 트루스: `skill/vertical-analysis/scripts/vertical_analysis.py` (이 문서의 코드는 사본)
> 입력: fit-parser 스킬의 `<name>_compact.csv` (30초 구간 집계)

## 구조: 탐지 엔진 하나 + 리포트 모드 둘

파싱·상승 블록 탐지는 공용이고, 해석만 모드로 갈린다.

| 모드 | 대상 | 리포트 |
|---|---|---|
| workout | 훈련 (총 180분 미만, 자동) | 버티컬 판정 + 전후 피로 비교 |
| race | 대회 (총 180분 이상, 자동. 짧은 대회는 `--mode race`) | 코스 구조 + 시간대별 운영 + 정지(CP) + 위험 신호 |

훈련 중 휴식이 CP 패턴과 구분되지 않아 정지 여부는 모드 판별에 쓰지 않는다(아차산 오판 사례로 확인).

## 지표 정의

| 지표 | 정의 | 용도 |
|---|---|---|
| VAM (m/h) | 구간 고도 변화 × (60 / 구간 분) — 시간당 상승 미터 | 등반 강도의 지형 독립 지표 |
| 상승 블록 | VAM ≥ 400 이고 경사 ≥ 3%가 연속되는 구간. 1.5분(3구간) 이내 끊김은 병합, 상승 30m 미만 제외 | 코스의 등반 구조 파악 |
| 버티컬 | 상승 50m 이상 블록 중 VAM 최대 **이면서 HR 최고 ≥ 세션 최고 × 0.93** | "의도적 최대치 등반" 탐지. HR 조건이 대회의 '그냥 가파른 지형'을 걸러냄 |
| 내리막 HR 비교 (workout) | 경사 -8% 이하 구간 평균 HR을 버티컬 전/후 비교. **+10bpm 이상 = 피로 신호** | 회복 부전 탐지 |
| 평탄 디커플링 (workout) | 경사 ±3% 구간의 NGP(없으면 페이스)·HR을 전/후 비교. **페이스 하락 + HR +5bpm 이상 = 신호** | 출력 대비 심박 상승 탐지 |
| 시간대별 운영 (race) | 60분 단위 이동 km·HR 평균·상승량, HR 드리프트(첫↔마지막 1시간) | 페이스 운영·출력 저하 곡선 |
| 정지 구간 (race) | 30초에 20m 미만 이동이 3분 이상 연속 | CP 휴식·중단 지점 탐지 |
| 위험 신호 (race) | 경사 -8% 이하를 세션 평균 HR+20 이상으로 주행한 구간, 전반/후반 분리 | 피로 상태 + 이심성 제동 = 둔근류 부상 위험 구간 |

## 검증 사례

- **아차산(2026-06-13, 훈련)**: workout 자동 판별. 버티컬 129~145.5분 +248m VAM 874, HR 185(세션 최고의 100%) — 러너의 "특정 업힐 최대치" 진술과 일치. 내리막 HR +16bpm, 평탄 디커플링 — "이후 운영이 힘들었다"는 체감과 일치
- **KOREA 50K(2026-04-25, 대회)**: race 자동 판별(384분). 최대 VAM 블록의 HR이 세션 최고 대비 낮아 버티컬 비판정(의도 없는 가파른 지형). 정지 5회 탐지(수동 분석에서 놓친 113분·256분 포함), 마지막 정지 = DNF 지점. 위험 신호 후반 30개 구간
- **호암산(훈련)**: 초반 버티컬 판정 + '전 구간 표본 부족' 경고
- **로드 러닝(여의도·보라매)**: "상승 블록 없음 — 평지 러닝"

## 한계

- 30초 집계 기반이라 1분 미만의 짧은 파워클라임은 뭉개질 수 있음
- 버티컬이 러닝 초반(15분 이내)이면 '전' 구간 표본 부족으로 전후 비교 신뢰도 낮음 (경고 표시)
- 전후 비교는 지형이 크게 다르면 피로가 아닌 지형 차이가 섞임
- 3시간 미만의 대회는 자동 판별이 workout으로 흐름 — `--mode race` 지정 필요
