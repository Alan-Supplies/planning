...
3. Read the most recent previous daily note, normally yesterday's `daily/YYMMDD.md`.
4. Read today's `daily/YYMMDD.md` if it exists.
5. Check whether the previous daily note records what actually happened. Treat the record as incomplete when planned work has no outcome and the `완료 및 결과` or `종료 점검` section is empty or still contains template placeholders. Do not infer completion from plans, unchecked boxes, or uncommitted files.
6. If the previous day's actual progress is missing, ask one focused question before creating or finalizing today's note. Name the previous day's primary task and any concrete result already visible in the note. Ask what was completed, what remains, and the next action without presenting a long questionnaire.
7. After the user answers, update the previous daily note with the smallest accurate change:
...
8. Use the corrected previous-day state when creating or updating today's note and briefing. If the user reports no progress, record that fact and carry forward only a concrete next action rather than copying the whole plan blindly.
9. Follow links only when the documents do not reveal the current position or the next concrete action.
...
11. Check whether another recent work change may be missing. Treat any of these as a signal, not proof:
...
12. If a missing change could materially alter today's briefing, ask one focused catch-up question before finalizing it. Name the specific task and the suspected gap; do not ask a generic question such as “업데이트 있나요?”. When useful, mention two or three likely statuses in plain language. Wait for the answer, then reconcile it with the documents and continue the briefing.
13. If the gap is minor or the available evidence is sufficient, give a provisional briefing and put one concise confirmation question at the end instead of blocking progress.
14. Give the briefing in Korean unless the user is speaking another language.
