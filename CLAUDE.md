...
- `project/데이터자산화/supplies-data-plan.md`(4000줄+ 원문)는 기본적으로 전체를 읽지 않습니다. 같은 폴더의
  `supplies-data-plan-summary.md`(요약본)나 `alan-실행순서.md`(실행 관점 발췌)를 먼저 참조하고, 원문은
  특정 WP·섹션 번호를 알 때만 `grep -n`으로 줄 번호를 찾아 그 부분만 `Read`(offset/limit)합니다.
- 이 패턴(원문 + `-summary.md` 요약본)이 있는 다른 문서도 동일하게 다룹니다 — 요약본을 기본 컨텍스트로
  쓰고, 원문은 근거 확인·인용이 필요할 때만 부분 발췌합니다.
- 일반적으로 수백 줄을 넘는 파일은 전체를 한 번에 읽기보다 `grep`으로 관련 구간을 먼저 찾고 그 근처만
  읽습니다. 사용자가 계획의 세부 내용을 전부 파악하고 있지 않은 상태로 대화가 이어지는 경우가 많으므로,
  매번 원문 전체를 다시 불러오지 말고 이미 파악한 요약·이전 대화 맥락을 우선 활용합니다.
